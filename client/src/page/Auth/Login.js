...
const Login = () => {
    ...
    const handleSubmit=async(e)=>{
        ...
        try{
            const res=await axios.post(`${process.env.REACT_APP_API}/auth/login`,{name,email,password});
            if(res.data.success){
    ...
    <div>
        ...
        <form className='infoform auth-form' onSubmit={handleSubmit}>
            ...
            <div>
                <input type="text" placeholder='Username' className='infoInput user' name='name' value={name} onChange={(e)=>setName(e.target.value)} />
                </div>
                <div>
                <input type="email" placeholder='Email' className='infoInput user' name='email' value={email} onChange={(e)=>setEmail(e.target.value)} />
                </div>
                <div>
